...
const HDGridConsumptionTabs = (props: any) => {
  ...
  const pm = tower1ConsumptionDataFormate(data[1]?.mainMeter, "yhat");
  const predictionData = pm?.map((num: any) => num.toString());
  const pmData = stringsBefore7?.concat(predictionData);

  const pdates = tower1ConsumptionDataFormate(data[1]?.mainMeter, "ds", true);
  const lengthP = pdates?.length;
  const overlDates = lastSevenDates?.concat(pdates);
  console.log("tool", overlDates, pmData);
    ...
    <>
      <div className="consumption-dashboard-tabs">
        <div style={{ display: "flex", justifyContent: "space-around" }}>
          <h1 className="substation-heading">Energy Meter Consumption</h1>
          <Tabs
            value={value}
        ...
        <TabPanel value={value} index={0}>
          <MultiLineChartYaxis
              ...
              data: [
                {
                  ...
                  type: "line",
                  data: showMorePrediction ? pmData : lastSevenStrings,
                },
            ...
            isCompare={isCompare}
            foreCast={showMorePrediction ? lengthP : []}
            isSingleLine={true}
          />
        </TabPanel>
      ...
      {showMorePrediction ? (
        <div className="htpreditionTable">
          <table>
            <tr>
              <th style={{background: "#f2f2f2"}}>Date</th>
              {data[1]?.mainMeter?.map((each: any) => {
                return (
                  <td style={{ background: "#f2f2f2" , padding:"12px", fontWeight:"600" }}>
                    {dateFormates(each.ds)}
                  </td>
                );
              })}
            </tr>
            ...
            <tr>
              <th style={{ paddingRight:"5px"}}>Mainmeter values</th>
              {data[1]?.mainMeter?.map((e: any) => {
                return <td style={{padding:"12px"}}>{e.yhat.toFixed(2)}</td>;
              })}
            </tr>
          </table>
        </div>
