...
prediction = async (data, params, callback) => {
  if (data.length !== 0) {
    ...
    try {
      const formattedData = rawData.map(({ date, consumption }) => ({
        date: moment(date, "MM/DD/YY HH:mm").format("YYYY-MM-DD HH:mm:ss"),

        consumption,
      }));

      let data1 = { data: formattedData };
      ...
      let python = spawn("python", [
        path.join(process.cwd(), "./", "app/scripts/dashboard.py"),
